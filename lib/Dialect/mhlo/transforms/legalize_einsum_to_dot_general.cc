...
struct EinsumToDotGeneralPattern : public OpRewritePattern<EinsumOp> {
  ...
  LogicalResult matchAndRewrite(EinsumOp einsum,
    ...
    assert(lhs_tokens.size() == lhs_type.getRank());
    assert(rhs_tokens.size() == rhs_type.getRank());

    auto collect_operand_dims =
        [&](RankedTensorType operand_type, SmallVector<char> operand_tokens,
            SmallVector<char> others,
            SmallVectorImpl<int64_t> &contracting_dims,
            SmallVectorImpl<int64_t> &batching_dims,
            SmallVector<char> &non_contracting_tokens,
            SmallVector<int64_t> &non_contracting_shape) {
          ...
          for (const auto &en : llvm::enumerate(operand_tokens)) {
            ...
            if (!isResultToken) {
              ...
              non_contracting_tokens.push_back(en.value());
              non_contracting_shape.push_back(
                  operand_type.getShape()[en.index()]);
            }
          }
        };
